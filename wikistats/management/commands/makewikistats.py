...
def downloadAllFiles(fileLinkTuples, previouslyProcessedFiles, lookbackPeriod):
    ...
    for tup in fileLinkTuples:
        ...
        print (datetime.datetime.now() + "start readFilesAndAggregateCounts for " + filename )
        allArticlesForGivenFileName = readFilesAndAggregateCounts([
                filename
            ], startDate, lookbackPeriod)
        print (datetime.datetime.now() + "end readFilesAndAggregateCounts for " + filename )
...
def doStuff():
    ...
    #allArticlesFromDB=ArticlePerDayStats.objects.all().delete()
    #return
    global allArticles
    #pdb.set_trace()
    allArticles = readAllArticlePerDayStatsFromDB()

    dumpArticlePerDayStats(allArticles)
    ...
    saveAllArticlePerDaySyaysToDB(allArticles)

    ##allArticles = readFilesAndAggregateCounts(["pageviews-20170729-230000"], startDate, 2)#lookbackperiod
    ##allArticles = readFilesAndAggregateCounts(["pageviews-20170729-100000"], startDate, 2)#lookbackperiod
    #allArticles = readFilesAndAggregateCounts([
    #        "wikistats/x/pageviews-20170729-100000",
    #        "wikistats/x/pageviews-20170729-230000",
    #        #"wikistats/x/pageviews-20170729-230000",
    #        "wikistats/x/pageviews-20170728-030000"
    #    ], startDate, 2)#lookbackperiod
